...
class Wav2Vec2(nn.Module):
    def __init__(self, n_blocks: int, n_mel_channels: int, d_model: int, heads: int, kernel_size: int, proj_dim: int, num_groups: int = 2, num_vars: int = 320, time_augment: int = 35, time_mask_ratio: float = 0.065, dropout_rate: float = 0.0) -> None:
        super().__init__()
        ...
        self.masker = TimeMasking(time_mask_param=time_augment, iid_masks=True, p=time_mask_ratio)

        self.quantization = Quantization(d_model=d_model, n_mel_channels=n_mel_channels, proj_dim=proj_dim, num_groups=num_groups, num_vars=num_vars)
    ...
    def forward(self, x: torch.Tensor, lengths: Optional[torch.Tensor] = None):
        ...
        context = self.masker(x.transpose(1, 2)).transpose(1, 2)

        target, perplexity = self.quantization(x)
        ...
        return context, target, perplexity