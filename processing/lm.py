...
class KenLanguageModel:
    def __init__(self, 
                 ...
                 hotwords: List[str] =  ['fpt', 'wifi', 'modem', 'fpt telecom', 'internet', 'free',  'lag', 'tổng đài viên', 'reset', 'check', 'test', 'code', 'port', 'net', 'email', 'mail', 'box'],
    ...
    def decode(self, logits: np.ndarray, decode_func: Optional[Callable[[str], str]] = None) -> str:
    ...
    def __call__(self, logits: np.ndarray, lengths: Optional[np.ndarray] = None, decode_func: Optional[Callable[[str], str]] = None) -> List[str]:
        return_str = False
        if logits.ndim == 2:
        ...
        for index, probs in enumerate(logits):
            text = self.decode(probs[: lengths[index], :] if lengths is not None else probs, decode_func=decode_func)
            preds.append(text)
